...
class VBertEmbeddings(BertEmbeddings):
        """Construct the embeddings from word, position and vgene embeddings."""
        ...
        def forward(
            ...
            vgene_embeddings = self.vgene_embeddings(vgene_ids)
            embeddings = inputs_embeds + token_type_embeddings + vgene_embeddings
...
class VBertModel(BertModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = VBertEmbeddings(config=config)
...
class VBertForMaskedLM(BertForMaskedLM):
    def __init__(self, config):
        super().__init__(config)

        if config.is_decoder:
            logger.warning(
                "If you want to use `VBertForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.bert = VBertModel(config, add_pooling_layer=False)
...
class VBertForSequenceClassification(BertForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
...
def get_bert_model(**kwargs) -> nn.Module:
    """
    Load the vbert model
    """
    logging.info("Loading VBERT model") 
    config = BertConfig(
        **kwargs,
        vocab_size=len(tk.AMINO_ACIDS_WITH_ALL_ADDITIONAL),
        pad_token_id=tk.AMINO_ACIDS_WITH_ALL_ADDITIONAL_TO_IDX[tk.PAD],
    )
    model = VBertForMaskedLM(config)
    return model